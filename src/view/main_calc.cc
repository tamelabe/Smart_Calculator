...
s21::MainCalc::MainCalc(QWidget *parent)
    : QWidget(parent), ui_(new Ui::MainCalc) {
  initElements();
  connectSlots();
}

s21::MainCalc::~MainCalc() { delete ui_; }

void s21::MainCalc::connectSlots() {
  QShortcut *sc_backspace =
  ...
  connect(ui_->label_x_activate, SIGNAL(clicked()), this,
          SLOT(activateLabelX()));
}
...
void s21::MainCalc::addSymbol() {
  if ((ui_->label_size->text()) == "255" && label_ == ui_->label_result) return;
  if ((label_->text().size()) >= 22 && label_ == ui_->label_x) return;
  if (label_->text() == "0") return;
...
void s21::MainCalc::createGraph() {
  ...
  std::pair<std::vector<double>, std::vector<double>> vector =
      controller_.getGraphVector(x_begin, x_end, y_begin, y_end);
  ...
  ui_->graph_window->graph(0)->setScatterStyle(
...
void s21::MainCalc::hideGraph() { this->setFixedSize(760, 327); }
...
void s21::MainCalc::typeFunctions() {
  ...
  if ((ui_->label_size->text().toInt() + (new_result.size())) > 255) return;
  if (label_->text() != "0") new_result = label_->text() + new_result;
  label_->setText(new_result);
  ui_->label_size->setText(QString::number(ui_->label_result->text().size()));
}

void s21::MainCalc::deleteLastSym() {
  if (label_->text() == "0" || label_->text() == "") return;
  QString new_result = label_->text();
  new_result = new_result.left(new_result.length() - 1);
  label_->setText(new_result);
  ui_->label_size->setText(QString::number(ui_->label_result->text().size()));
  if (ui_->label_result->text() == "") ui_->label_result->setText("0");
}

void s21::MainCalc::calculate() {
  if (ui_->label_result->text() == "0") return;
