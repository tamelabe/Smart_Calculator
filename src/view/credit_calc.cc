...
void s21::CreditCalc::setCalculatedValues() {
    ...
    if (ui_->box_type->currentText() == "Differentiated" && ui_->box_period->value() > 1) {
        QString month_pay_back = QString::fromStdString(controller_.getResult().month_pay.back());
...
void s21::CreditCalc::fillCeils() {
    ...
    for (int i = 0; i < ui_->box_period->value(); ++i) {
        QTableWidgetItem *val  = new QTableWidgetItem(QString::fromStdString(month_pay_v_[i]));
...
void s21::CreditCalc::hideGraph() {
    this->setFixedSize(280, 500);
}

void s21::CreditCalc::deallocateCeils() {
    if (ui_->tableWidget->item(0, 0) == nullptr) { return; }
