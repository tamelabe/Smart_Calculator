...
struct CreditParams {
  ...
  ~CreditParams() = default;
 public:
  CreditParams(const std::string &type_i, double loan_i, int period_i, double rate_i) :
  type(type_i), loan(loan_i), period(period_i), rate(rate_i) {}
  void setInput(const std::string &type_i, double loan_i, int period_i, double rate_i)  {
    type = type_i;
    loan = loan_i;
    period = period_i;
    rate = rate_i;
